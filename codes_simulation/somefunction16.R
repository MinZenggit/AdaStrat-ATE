...
# The error-prone influence functions was calculated using the 
# incompelete data (x_c, treatment, y) to estimate the ate.

######FixStrat#################
# o: Stratified by outcome (FixStrat)

source("~/Downloads/ODS_06_01/codes_ate_influence_functions/esATE.R")
source("~/Downloads/ODS_06_01/codes_ate_influence_functions/esATE_w2.R")
...
testf <- function(N, n_m, n, K, method){
  ...
  #####
  # best optimal
  getpi(dt1$phi_a, (n+n_m))*(n+n_m) -> dt1$pi_b
  ...
  te2 <- te
  getpi_stra(sqrt(te2), N1_2, n, K, p = rep(1/K, K)) -> pi_m
  dt2$pi_m = NA
  for(k in 1:K){
  ...
  te2 <- te
  getpi_stra(sqrt(te2), N1_2, n, K, p = rep(1/K, K)) -> pi_O
  ...
  return(c(fit_u, fit_b, fit_Z, fit_M, fit_ZM, fit_O))
}
